...
- **Status:** v1.0.9 — Phase 6.7 write-side exposure per ADR-025 (Slice 2 of 3 toward v1.1.0). `MCP_EXPOSED_TOOL_NAMES` is now dynamic — `mcpExposedToolNames({ writeEnabled, highRiskEnabled })` returns 5 read tools always, 9 non-high-risk write tools when `mcpWriteEnabled`, plus `delete_note` when `mcpHighRiskToolsEnabled`. New `src/mcp/WriteGate.ts` runs five gates per write call: master toggle (D1), high-risk (D1), per-client allowlist (D6), path-scope (D7 — every path arg checked, including `from`/`to` for moves), rate limit (D9 — rolling 60-min cap via `WriteRateLimiter`). `McpHandler` opens a transaction tagged `source: 'mcp:<client>'` via the shared `WriteToolContext.begin(undefined, clientName)` so successful writes carry full provenance into `Transaction.source`. Concurrent in-app chat + MCP write conflict yields a clear "in-app chat in progress, retry shortly" response (no preempt). 910 tests (+31). Next: v1.1.0 lands the queue + side panel + OS notifications. ADR-026 (Phase 8) MVP at v1.2.0.

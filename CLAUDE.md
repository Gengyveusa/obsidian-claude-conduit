...
- **Status:** v1.0.6 — Curator `normalize-tag` apply path landed. Applying a tag-cluster suggestion now scans every `.md`, finds notes still using a non-canonical cluster member, and proposes one diff-card-gated `patch_note` per affected note (one card at a time, sequential). The rewrite covers inline `#tag` body references and YAML frontmatter forms (inline-array, scalar, block-list). Closes the third quarter of ADR-024 follow-ups; combined with v1.0.4 (LLM-judged wiring) and v1.0.5 (SkipPatternStore) this leaves only `duplicate-candidate` merge-into as deferred (needs new `delete_note` tool + modal + sequential-write combo — earmarked for v1.0.7 or Phase 8 when generative-merge tooling lands). All six detectors live in `runCurator`. Next big steps: Phase 6.7 (MCP write-side, needs ADR-025) and Phase 8 (generative layer, needs ADR-026).

...
> **Status:** v0.2.5 — chat-mode + 5 vault tools (`read_note`, `list_folder`, `search_vault`, `get_backlinks`, `get_graph_neighborhood`) + `Sagittarius: System check` command (v0.2.4) for live-stack health. Side panel, Cmd+P modal, settings tab, conversation logging to vault, daily token + dollar budget caps, model fallback. **Semantic retrieval via the official [`@huggingface/inference`](https://www.npmjs.com/package/@huggingface/inference) SDK per [ADR-013](docs/2026-05-08-hf-inference-embedding-strategy.md) postscript #3 (v0.2.5).** Routed through Obsidian's `requestUrl()` to dodge renderer CORS (v0.2.1 fix). v0.2.3 swapped the indexer's recursive `adapter.list()` walker for `app.vault.getMarkdownFiles()`. Without an HF token, the plugin gracefully degrades to chat-mode + 4 vault-API tools.

...
> **Status:** v0.2.3 — chat-mode + 5 vault tools (`read_note`, `list_folder`, `search_vault`, `get_backlinks`, `get_graph_neighborhood`). Side panel, Cmd+P modal, settings tab, conversation logging to vault, daily token + dollar budget caps, model fallback. **Semantic retrieval re-enabled via the HuggingFace Inference Providers router per [ADR-013](docs/2026-05-08-hf-inference-embedding-strategy.md).** v0.1.x's transformers.js path is gone (didn't survive Obsidian's Electron renderer per ADR-012); v0.2 routes embeddings through `router.huggingface.co/hf-inference/...` (v0.2.2 endpoint update; legacy `api-inference.huggingface.co` was retired) via Obsidian's `requestUrl()` to dodge renderer CORS (v0.2.1 fix; see ADR-013 postscripts). v0.2.3 swaps the indexer's recursive `adapter.list()` walker for `app.vault.getMarkdownFiles()` — the walker silently terminated when `list('')` threw on production Obsidian, leaving every build at 0 notes. Without an HF token, the plugin gracefully degrades to v0.1.1 behavior (chat-mode + 4 tools).
